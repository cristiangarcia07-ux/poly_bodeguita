...
CREATE TABLE platos_pedidos (
    ...
    tipo_racion VARCHAR(10) NOT NULL,
...
BEGIN
    ...
    WHILE i < num_pedidos DO
        ...
        WHILE j < num_platos DO
            ...
            -- Usar INSERT IGNORE previene que falle si escoge exactamente el mismo plato
...
BEGIN
    ...
    ELSE
        ...
        ON DUPLICATE KEY UPDATE 
            ...
            tipo_racion = VALUES(tipo_racion);
...
INSERT INTO pedidos (total, subtotal, cliente_id, empleado_id, esadomicilio, direccion_id, abierto)
SELECT 15.00, 15.00, c.id, 1, TRUE, d.id, FALSE
FROM clientes c
JOIN Direcciones d ON d.Direccion = 'Calle Larios 12'
CROSS JOIN (SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5) n1
CROSS JOIN (SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5) n2
WHERE c.email = 'jeremiaselcocas@gmail.com' LIMIT 25;
-- no case whens when creating a procedure, only if while should be allowed
-- ==========================================================
-- VISTAS Y FUNCIONES ADICIONALES (Estadísticas)
-- ==========================================================

-- 1. Vista de Rentabilidad de Platos
...
SELECT id, total, cliente_id, empleado_id, esadomicilio, fecha, time
FROM pedidos
ORDER BY total DESC
LIMIT 10;
